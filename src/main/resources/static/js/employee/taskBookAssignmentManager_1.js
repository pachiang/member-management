function TaskBookAssignmentManager() {
    ...
    // get current avalible list and its option list
    const [currentAvalibleList, setCurrentAvalibleList] = React.useState([]);
    let optionList = currentAvalibleList.map((optionEle)=><option value={optionEle.eid} key={optionEle.eid}>{optionEle.firstName + ' ' + optionEle.lastName}</option>)
    ...
    const handleSelectClicked = async (book) => {
        const { data: avalibleListOfElement } = await axios.get(avalibleEmployeeByMidApiEndPoint + book.book_id)
        setCurrentAvalibleList(avalibleListOfElement)
        console.log(avalibleListOfElement)
    }
		...
		return (<tr className="hover" key={ele.book_id}>
			...
			<td>{ele.f_customer_id ? getTheCusName(ele.f_customer_id) : ''} {ele.f_customer_id ? (
				<div className="dropdown dropdown-top dropdown-hover">
					<label tabIndex={0}><svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 mx-2" viewBox="0 0 24 24" fill="gray" stroke="gray" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><g fill="none" fillRule="evenodd"><path d="M18 14v5a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8c0-1.1.9-2 2-2h5M15 3h6v6M10 14L20.2 3.8" /></g></svg></label>
			...
			<td>
                <select disabled={ele.status === 'closed' ? true : false} onClick={()=>handleSelectClicked(ele)} onChange={()=>handleChange(ele)} value={selectedValue} className="select select-bordered w-full max-w-xs"><option value="default" disabled selected>
